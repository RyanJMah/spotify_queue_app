...
def delete_queue(session_id):
	db = connect()
	cursor = db.cursor()
	sql = f"DELETE FROM queues WHERE session_id = '{session_id}'"
	cursor.execute(sql)
	db.commit()
	db.close()
	return

def read_queue(session_id = None):
	db = connect()
	if (session_id == None):
...
def add_queue(session_id, guest_user, song_id, time):
	db = connect()
	cursor = db.cursor()
	sql = "INSERT INTO queues(session_id, guest_user, song_id, time) VALUES (%s, %s, %s, %s)"
	val = (session_id, guest_user, song_id, time)
...
def add_session(session_id, host_token):
	db = connect()
	cursor = db.cursor()
